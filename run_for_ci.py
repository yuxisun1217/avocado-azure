...
#        ret += command("/usr/bin/python {0}/xen-ci/utils/import_XunitResult2Polarion.py -f {1}/merged_result.xml -d {0}/xen-ci/database/testcases.db -t azure -p {2} -r \"{3}\" -o {1}/xUnit.xml -k {4}".format(AVOCADO_PATH, result_path, xunit_project, TESTRUN_PREFIX, CIUSER_PASSWORD), debug=True, stdout=True).exit_status
        ret += command("/usr/bin/python {0}/xen-ci/utils/import_XunitResult2Polarion.py -f {1}/merged_result.xml -d {0}/xen-ci1/database/testcases.db -t azure -p {2} -r \"{3}\" -o {1}/xUnit.xml -k {4}".format(AVOCADO_PATH, result_path, xunit_project, TESTRUN_PREFIX, CIUSER_PASSWORD), debug=True, stdout=True).exit_status
