...
def login_azure(username, password):
    ...
    for retry in xrange(1, 11):
        logging.info("azure login -u %s -p ******" % username)
...
#        login_ret = command("azure account show --json")
        if login_ret.exit_status == 0:
            logging.info("Login successfully with: %s", username)
            return login_ret.exit_status
        else:
            if login_ret.exit_status == 143:
                logging.info("Login timeout. Retry: %d/10" % retry)
                time.sleep(10)
...
def logout_azure(username):
    ...
    logging.info("Logout Azure with: %s", username)
#    cmd = shlex.split(cmd)
#    try:
...
#        raise Exception("Failed to logout with: %s", username)
    ...
    if ret.exit_status == 0:
        logging.info("Logout successfully with: %s", username)
        return 0
    else:
...
def account_clear(options=''):
    ...
    logging.info("Clear Azure account")
#    cmd = shlex.split(cmd)
#    try:
#        subprocess.check_call(cmd)
#    except:
...
def set_config_mode(mode="asm"):
    ...
    :return: True if operate successfully
    """
    logging.info("Change the azure config mode as %s", mode)
...
#        raise
#    else:
#        logging.info("Success to change the azure config mode: %s", mode)
#        return True
    ret = command(cmd, timeout=60)
    if ret.exit_status == 0:
        logging.info("Succeed in changing azure config mode: %s", mode)
        return ret.exit_status
    else:
