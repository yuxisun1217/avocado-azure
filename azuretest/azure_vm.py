...
#        return self.params.get('Network').get('Endpoints')[0].get('port')
    ...
    def get_params(self, key):
        ...
        try:
            ...
            logging.info("Cannot get value of key: %s", key)
            raise
        return value
...
#            cmd = "echo %s | sudo -S sh -c \"%s\"" % (self.password, cmd)
        for retry in xrange(0, max_retry+1):
            try:
                ...
                logging.info("Run command %s timeout. Retry %d/%d" % (cmd, retry, max_retry))
                self.verify_alive()
                continue
            except Exception, e:
            ...
            if ignore_status:
                ...
                logging.info("After retry %d times, run command %s timeout. Exception: %s" % (retry, cmd, e))
                raise
        logging.info(output)
        return output
