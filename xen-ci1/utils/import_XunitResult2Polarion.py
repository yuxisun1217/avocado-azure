...
#    ret = commands.getstatusoutput(cmd)
    ...
    while True:
        ...
        if timeout and seconds_passed > timeout:
            p.terminate()
            logging.error("After {0} seconds, cannot get messages from ci-bus. Exit.".format(seconds_passed))
            sys.exit(1)
        time.sleep(1)
...
class ResultConvert(object):
    ...
    def convert_result(self):
        ...
        for testcase_node in testsuite_node.findall("./testcase"):
            ...
            if TEAM == "azure":
                ...
                comment = long_name
                config = testcase_node.attrib.get("azuremode", "ASM")
    ...
    def import_result(self):
        """
        Import result to polarion
        """
        import_file = OUTPUTFILE if OUTPUTFILE.startswith("/") else './'+OUTPUTFILE
        ret = _run("curl -k -u {0}_machine:polarion -X POST -F file=@{1} \"https://polarion.engineering.redhat.com/polarion/import/xunit\"".format(PROJECT, import_file))
        if (ret[0] != 0) or ("Error report" in ret[1]):
            logging.error("Fail to post result to polarion. Exit.")
            sys.exit(1)
        ret = _run("python subscribe.py --user ci-user --password {0} --selector \"myteamsname='{1}qe'\" --count 1".format(PASSWORD, TEAM))
        if (ret[0] != 0) or (TEAM not in ret[1]):
            logging.error("Fail to get return messages from ci-bus. Exit.")
            sys.exit(1)
        output = ret[1].strip('\n')
        body_line_num = output.split('\n').index("Message body:")
        import json
        return_json = json.loads('\n'.join(output.split('\n')[body_line_num+1:-1]))
        if return_json.get("status") != "passed":
            logging.error("Import status is not passed. Messages: {0}".format(output))
            sys.exit(1)
        testrun_url = return_json.get("testrun-url")
        log_url = return_json.get("log-url")
        logging.info("Import result successfully.")
        logging.info("Test run url: " + testrun_url)
        logging.info("Import log url: " + log_url)


if __name__ == "__main__":
    usage = "usage: %prog -p <project> -t <team> -r <testrun_prefix> -k <ci-user password> [-d <database> -f <autotest_xml> -o <output_xml>] [-v]"
    ...
    parser.add_option('-k', '--password', dest='password', action='store',
                      help='The password of ci-user, used to monitor ci-bus.', metavar='PASSWORD')
    ...
    if not PROJECT or not TEAM or not TESTRUN_PREFIX or not PASSWORD:
        parser.print_help()
        parser.error("Must specific project, team, testrun prefix and ci-user password!")
